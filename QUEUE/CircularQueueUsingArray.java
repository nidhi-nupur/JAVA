/**
 * CircularQueueUsingARRAY
 */
public class CircularQueueUsingArray {

    // A static inner class for Queue
    static class Queue {
        // Declaring static variables for the array, size, rear, and front
        static int array[];
        static int size;
        static int rear;
        static int front;

        // Queue Constructor initializes the array with given size and sets rear and front to -1
        Queue(int n) {
            array = new int[n];
            size = n;
            rear = -1;
            front = -1;
        }

        // isEmpty() Method - checks if the queue is empty by checking if rear and front are both -1
        public static boolean isEmpty() {
            return rear == -1 && front == -1;
        }

        // isFull() Method - checks if the queue is full
        public static boolean isFull() {
            return (rear+1) % size == front;
        }
        // Add Method - adds an element to the queue if it's not full
        public static void add(int data) { // TC = O(1)
            if(isFull()) { // Check if the queue is full
                System.out.println("Queue is Full");
                return; // Exit if full
            }

            // When adding 1st element
            if(front == -1) {
                front = 0;
            }
            
            rear = (rear + 1) % size; // Increment rear to the next position
            array[rear] = data; // Add the new element at the rear
        }

        // Remove Method - removes an element from the front of the queue
        public static int remove() { // TC = O(1)
            if(isEmpty()) { // Check if the queue is empty
                System.out.println("Empty Queue");
                return -1; // Exit if empty
            }

            int result = array[front]; // Store the front element
            // Last element delete
            if(rear == front) {
                rear = front = -1;
            } else {
                front = (front + 1) % size;
            }
            
            return result; // Return the removed front element
        }

        // peek Method - returns the front element without removing it
        public static int peek() { // TC = O(1)
            if(isEmpty()) {
                System.out.println("Empty Queue");
                return -1;
            }

            return array[front]; // Return the front element
        }
    }
    public static void main(String[] args) {
        Queue queue = new Queue(5); // Create a new Queue with a capacity of 5
        queue.add(1);
        queue.add(2);
        queue.add(3);
        System.out.println(queue.remove());
        queue.add(4);
        System.out.println(queue.remove());
        queue.add(5);
        // 1 2 3 4 5

        // Remove and print elements from the queue until it's empty
        while (!queue.isEmpty()) {
            System.out.println(queue.peek()); // Print the front element
            queue.remove(); // Remove the front element
        }
    }
}